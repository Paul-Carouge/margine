    ...
    return Scaffold(
      ...
      body: statsAsync.when(
        data: (stats) {
          ...
          final avgProfit =
              soldCount > 0 ? profit / soldCount : 0.0;
            ...
            children: [
              ...
              Text(
                ...
                style: tt.displayLarge?.copyWith(
                  color: profit >= 0 ? ForgeColors.teal : ForgeColors.error,
                ),
              ),
              ...
              _HighlightCard(
                ...
                value: roi >= 0
                    ...
                    : '${roi.toStringAsFixed(1)} %',
                color:
                    roi >= 0 ? ForgeColors.teal : ForgeColors.error,
                sublabel:
                    '${count} article${count > 1 ? 's' : ''} · ${invested.toStringAsFixed(0)} € investis',
                cs: cs,
              ),
              ...
              productsAsync.when(
                data: (products) =>
                    _BestWorstSection(products: products, cs: cs, tt: tt),
                loading: () => const SizedBox.shrink(),
                error: (_, __) => const SizedBox.shrink(),
              ),
...
class _BestWorstSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final sold = products
        .where((p) => p.status == 'sold' && p.salePrice != null)
        .toList();
    if (sold.isEmpty) return const SizedBox.shrink();

    // Calculate profit for each sold product
    final withProfit = sold.map((p) {
      final profit = p.salePrice! -
          p.purchasePrice -
          p.vintedFees -
          p.shippingCost -
          p.packagingCost;
      final marginPct =
          p.purchasePrice > 0 ? (profit / p.purchasePrice * 100) : 0.0;
